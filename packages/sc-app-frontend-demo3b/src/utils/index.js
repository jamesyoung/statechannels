...
const contractJSON = require('channel-contracts/build/contracts/Demo3b.json')
...
export async function openChannelSignedTx(opts) {
  const { value, timeout, account } = opts
  const web3 = new Web3(getProvider())
  const instance = new web3.eth.Contract(abi, address)

  const data = await instance.methods.deposit(timeout).encodeABI()
